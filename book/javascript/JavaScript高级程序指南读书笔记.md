...
采用弹窗阻止插件利用 window.open()的时候会抛出一个错误，需要封装到 try-catch 中
...
5.Element 类型判断节点类型(el.tagName)为了保险起见采用 toLowerCase()
...
6.事件模拟在 document 中使用 createEvent()方法创建 event 对象
